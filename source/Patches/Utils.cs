...
namespace TownOfUs
{
    ...
    public static class Utils
    {
        ...
        public static bool isGlitchBlocked(this PlayerControl player)
        {
            if (!player.Is(RoleEnum.Glitch)) return false;
            var killedPlayers = 0;
            foreach (var dead in Murder.KilledPlayers)
                if (dead.KillerId == player.PlayerId)
                    killedPlayers++;
            return killedPlayers < CustomGameOptions.GlitchShieldUntilKills;
        }

        public static bool isImpBlocked(this PlayerControl player)
        {
            ...
            foreach (var dead in Murder.KilledPlayers)
                if (dead.KillerId == player.PlayerId)
                    killedPlayers++;
            return killedPlayers < CustomGameOptions.ImpShieldUntilKills;
        }
